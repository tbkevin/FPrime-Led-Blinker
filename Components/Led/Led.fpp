module Components {
    @ Component to blink a LED driven by a rate group
    active component Led {
        ...
        @ Reports the state we set the blink interval parameter.
        event BlinkIntervalSet(interval: U32) \
            severity activity high \
            format "LED blink interval set to {}."

        @ Reports the state when the LED is driven to a new state
        ...
        @ Telemetry channel to report number  LED transitions
